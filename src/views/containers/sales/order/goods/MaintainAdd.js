...
class MaintainAdd extends Component {
  // 构造函数，在创建组件的时候调用一次
  constructor(props) {
    super(props);
        ...
        <Row>
          {
            item.options.map((item, index) => {
              ...
              item.options.disabled = this.props.isReview&&this.state.goodsReviewSure || item.disabled;
  ...
  render() {
    ...
    let maintainSize = {};
    if (this.props.data) {
      let data = {...this.props.data, ...maintainSize};
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        if (this.props.onAddSuccess) {
          ...
          let maintainInfo = values;
          if (maintainInfo.urgent) {
            maintainInfo.urgent = this.getValueFromListById(this.props.sales.urgentList, maintainInfo.urgent);
          }
          if (this.state.pics) {
          ...
          this.props.onAddSuccess(maintainInfo);
          return true;
        }
      }
    })
  ...
  onNIDSelect = (value, option) => {
    for(let i=0; i<this.props.sales.maintainList.length; i++) {
      let maintain = this.props.sales.maintainList[i];
      if (maintain.NID === value) {
        const {form:forms} = this.props;
        forms.setFieldsValue({NID:maintain.NID});
        forms.setFieldsValue({m_name:maintain.name});
        forms.setFieldsValue({m_price:maintain.price});
        forms.setFieldsValue({m_time:maintain.time});

        this.setState({selectGoods:maintain});
      }
    }
  }
